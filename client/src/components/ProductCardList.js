  ...
  return (
    <>
      <div tw="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        {type === 'All'
          ? allProducts?.map((product) => <ProductCard product={product} wishlistIdArray={wishlistIdArray}  />)
          : products?.map((product) => <ProductCard product={product} wishlistIdArray={wishlistIdArray} />)}
      </div>
    </>
  );
};
